...
namespace ClassicUO.Game.Managers
{
    internal class BandageManager
    {
        ...
        {
            if (World.Instance.Player == null || !isEnabled)
                return;

            // Guard against divide-by-zero
            if (World.Instance.Player.HitsMax <= 0)
                return;

            var currentHpPercentage = (int)((double)newHp / World.Instance.Player.HitsMax * 100);

            // Check for hidden status
            if (CheckHidden && World.Instance.Player.IsHidden)
                return;

            // Check for poison status #thanks taz
            if ((!UseOnPoisoned || !World.Instance.Player.IsPoisoned) &&
